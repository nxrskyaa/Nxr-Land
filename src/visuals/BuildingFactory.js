...
export class BuildingFactory {
  constructor(materials) { this.materials = materials; }

  starterHouse(x, z) {
    const group = new THREE.Group();
    const wall = mesh(GEO.wall, this.materials.cream, [0, 1.15, 0]);
    wall.scale.set(1, 1, 0.95);
    const roof = mesh(GEO.roof, this.materials.roof, [0, 2.8, 0]);
    roof.rotation.y = Math.PI / 4;
    roof.scale.set(1, 0.7, 0.82);
    const door = mesh(GEO.door, this.materials.peach, [0, 0.68, 1.38]);
    const knob = new THREE.Mesh(new THREE.SphereGeometry(0.07, 8, 6), this.materials.yellow);
    knob.position.set(0.22, 0.72, 1.49);
    const windows = [-1.05, 1.05].map((wx) => mesh(GEO.window, this.materials.blue, [wx, 1.35, 1.43]));
  ...
  marketStall(x, z, color = this.materials.coral) {
    const group = new THREE.Group();
    [-0.85, 0.85].forEach((px) => group.add(mesh(GEO.post, this.materials.woodDark, [px, 0.78, -0.45])));
    group.add(mesh(GEO.stallCounter, this.materials.wood, [0, 0.8, 0]), mesh(GEO.stallTop, color, [0, 1.7, -0.05]));
  ...
  fenceSegment(x, z, rotation = 0, length = 1.5) {
    const group = new THREE.Group();
    [-0.65, 0.65].forEach((px) => group.add(mesh(GEO.post, this.materials.wood, [px * length / 1.5, 0.5, 0])));
    [0.38, 0.78].forEach((y) => {
      const plank = mesh(GEO.plank, this.materials.cream, [0, y, 0]);
  ...
  expansionGate(x, z, labelColor = this.materials.gate) {
    const group = new THREE.Group();
    group.add(mesh(GEO.archPost, this.materials.gate, [-1.35, 1.1, 0]), mesh(GEO.archPost, this.materials.gate, [1.35, 1.1, 0]));
    const top = mesh(GEO.archTop, labelColor, [0, 2.15, 0]);
    top.rotation.z = -0.03;
    group.add(top);
  ...
  bench(x, z, rotation = 0) {
    const group = new THREE.Group();
    group.add(mesh(GEO.benchSeat, this.materials.wood, [0, 0.52, 0]), mesh(GEO.benchBack, this.materials.wood, [0, 0.84, -0.25]));
    [-0.55, 0.55].forEach((px) => group.add(mesh(GEO.post, this.materials.woodDark, [px, 0.26, 0])));
    group.position.set(x, 0, z);
    group.rotation.y = rotation;
    return setSoftShadows(group);
  }
  ...
  sign(x, z, rotation = 0) {
    const group = new THREE.Group();
    group.add(mesh(GEO.post, this.materials.woodDark, [0, 0.7, 0]), mesh(box(1.35, 0.65, 0.18), this.materials.yellow, [0, 1.2, 0]));
