...
function advanceToGarden(context) {
  return context.quests.record('location:entered', { locationId: 'town-plaza' });
}
...
describe('QuestSystem data-driven Chapter 1 flow', () => {
  ...
  it('persists partial progress across reload and never duplicates progress from one-shot world objects', () => {
    ...
    expect(second.state.quests.activeId).toBe('chapter-1-plant-crop');
  });
  ...
  it('does not mutate migration defaults in its constructor and rejects unsafe reward totals atomically', () => {
    ...
    expect(advanceToGarden(context)).toMatchObject({ ok: false, code: 'transaction-failed' });
  ...
  it('listens to real farming and economy event shapes but validates their semantic target', () => {
    ...
    advanceToGarden(context);
    completeGarden(context);
    context.eventBus.emit('crop:watered', { cropId: null, plotId: 'home-plot-1' });
    expect(context.state.quests.activeId).toBe('chapter-1-plant-crop');
    context.eventBus.emit('crop:watered', { cropId: 'carrot', plotId: 'home-plot-1' });
    expect(context.state.quests.activeId).toBe('chapter-1-reopen-market');
    context.eventBus.emit('economy:sold', { itemId: 'seed-turnip', item: { type: 'seed' }, quantity: 1 });
    expect(context.state.quests.activeId).toBe('chapter-1-reopen-market');
    context.eventBus.emit('economy:sold', { itemId: 'produce-carrot', item: { type: 'produce' }, quantity: 1 });
    expect(context.state.quests.activeId).toBe('chapter-1-find-spirit-seed');
  });
});
